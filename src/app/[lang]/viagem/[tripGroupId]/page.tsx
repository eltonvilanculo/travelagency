...
const COPY = {
  pt: {
    ...
    footerNote: "Este documento é uma cotação, não uma fatura. Os valores finais são confirmados por um dos nossos agentes antes do pagamento.",
  ...
  en: {
    ...
    footerNote: "This document is a quote, not an invoice. Final amounts are confirmed by one of our agents before payment.",
...
export default async function TripQuotePage({ params }: PageProps) {
  ...
  const reservations = await ReservationService.findByTripGroupId(tripGroupId);
