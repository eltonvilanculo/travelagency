  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <div className="relative" ref={ref}>
        ...
        {open && (
          <div className="absolute right-0 z-10 mt-1 w-48 rounded-lg border border-slate-200 bg-white shadow-lg py-1 origin-top-right animate-dropdown-in">
            {otherOptions.map((s) => (
              <button
                key={s}
                onClick={() => {
...
export function ReservasClient() {
  ...
  const handleCollectPayment = async (id: string) => {
    if (paymentMethod !== "TRANSFER" && !walletNumber.trim()) return;
      ...
      const response = await fetch(`/api/admin/reservations/${id}/payments`, {
        ...
        body: JSON.stringify(
          paymentMethod === "TRANSFER"
            ...
            : { method: paymentMethod, walletNumber: walletNumber.trim() }
        ),
      });
      if (!response.ok) {
        const body = await response.json().catch(() => ({}));
                    ...
                    <td className="px-4 py-3 text-right">
                      ...
                      >
                        ...
                        <svg
                          ...
                          stroke="currentColor"
                          strokeWidth={2}
                          className={`w-3.5 h-3.5 transition-transform duration-200 ${expandedId === r.id ? "rotate-180" : ""}`}
                        >
                            ...
                            {r.quotedPrice && r.payments.length === 0 && (
                              <div className="mt-3">
                                ...
                                <div className="flex gap-2">
                                  <select
                                    value={paymentMethod}
                                    onChange={(e) => setPaymentMethod(e.target.value as "MPESA" | "EMOLA" | "TRANSFER")}
                                  ...
                                  {paymentMethod === "TRANSFER" ? (
                                    <p className="flex-1 text-xs text-slate-500 self-center">
                                      Sem carteira — confirme manualmente quando a transferência chegar.
                                    </p>
                                  ) : (
                                    <input
                                      value={walletNumber}
